...
switch($_GET[act]){
  ...
  default:
    if ($_SESSION[leveluser]=='admin'){
      $tampil_admin = mysqli_query($con,"SELECT * FROM pengajar WHERE level='1' ORDER BY id_pengajar");      
      echo "<h2>Manajemen Administrator</h2><hr>";
          echo "<br><div class='information msg'>Account administrator tidak bisa di hapus, tapi bisa di non aktifkan.</div>";
          echo "<br><table  id='table1' class='gtable sortable'><thead>
          <tr><th>No</th><th>ID</th><th>Nama</th><th>Email</th><th>HP</th><th>Status</th><th>Aksi</th></tr></thead>";
    $no=1;
    while ($r=mysqli_fetch_array($tampil_admin)){
       echo "<tr><td>$no</td>
             <td>$r[id_pengajar]</td>
             <td>$r[nama_pengajar]</td>
             <td><a href=mailto:$r[email_pengajar]>$r[email_pengajar]</a></td>
             <td>$r[hp_pengajar]</td>";
             if($r[status_idpeng]=='0'){
              $status='Pending';
             }
             elseif($r[status_idpeng]=='1'){
              $status='Aktif';
             }
             else{
              $status='Blok';
		  ...
		  echo "<td>$status</td>
             <td><a href='?module=admin&act=editadmin&id=$r[id_pengajar]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a></td></tr>";
      $no++;
    }
    echo "</table>";
    }
  ...
  case "pengajar":
  if ($_SESSION[leveluser]=='admin'){
      $tampil_pengajar = mysqli_query($con,"SELECT * FROM pengajar WHERE level='2' ORDER BY id_pengajar");
    echo "<h2>Manajemen Pengajar</h2><hr>";
          echo "<br><table  id='table1' class='gtable sortable'><thead>
          <tr><th>No</th><th>ID</th><th>Nama</th><th>Email</th><th>HP</th><th>Status</th><th>Aksi</th></tr></thead>";
    $no=1;
    while ($r=mysqli_fetch_array($tampil_pengajar,MYSQLI_BOTH)){
       echo "<tr><td>$no</td>
             <td>$r[id_pengajar]</td>
             <td>$r[nama_pengajar]</td>
             <td><a href=mailto:$r[email_pengajar]>$r[email_pengajar]</a></td>
      ...
      if($r[status_idpeng]=='0'){
       ...
       echo"<td>$status</td>
             <td><a href='?module=admin&act=editpengajar&id=$r[id_pengajar]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a></td></tr>";
      $no++;
    }
    echo "</table>";
  }else{
  ...
  case "editadmin":
    $edit=mysqli_query($con,"SELECT * FROM pengajar WHERE id_pengajar='$_GET[id]'");
    $r=mysqli_fetch_array($edit,MYSQLI_BOTH);
    ...
    echo "<form method=POST action=$aksi?module=admin&act=update_admin>
          ...
          <dd> : <input type=text name='id' value='$r[id_pengajar]' readonly><br><small>ID tidak dapat diubah</dd>
          <dt><label>Nama</label></dt>
          <dd> : <input type=text name='nama' value='$r[nama_pengajar]'></dd>
          <dt><label>Username</label></dt>
          <dd> : <input type=text name='username' value='$r[username_pengajar]'></dd>
          <dt><label>Password</label></dt>
          <dd> : <input type=text name='password'>
          <small>Apabila password tidak diubah, dikosongkan saja.</small></dd>
          <dt><label>E-mail</label></dt>
          <dd> : <input type=text name='email' value='$r[email_pengajar]'></dd>
          <dt><label>HP :</label></dt>
          <dd> : <input type=text name='hp' value='$r[hp_pengajar]'></dd>";
          if($r[level]=='1'){
          echo "<dt><label>Level</label></dt>
          <dd> : <label><input type=radio name='level' value='1' checked>Admin</label>
          <label><input type=radio name='level' value='2'>Pengajar</label></dd>";}
          else{
          	echo "<dt><label>Level</label></dt>
          <dd> : <input type=radio name='level' value='1'>Admin
          <input type=radio name='level' value='2' checked>Pengajar</dd>";}
          if($r[status_idpeng]=='0'){
 ...
 case "editpengajar":
    ...
    elseif ($_SESSION[leveluser]=='pengajar'){
        $edit=mysqli_query($con,"SELECT * FROM pengajar WHERE id_pengajar='$_SESSION[idpengajar]'");
        $r=mysqli_fetch_array($edit,MYSQLI_BOTH);
...
case "detailpengajar":
    $detail=mysqli_query($con,"SELECT * FROM pengajar WHERE id_pengajar='$_GET[id]'");
    $r=mysqli_fetch_array($detail,MYSQLI_BOTH);
