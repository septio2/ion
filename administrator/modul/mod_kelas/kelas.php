  ...
  default:
    if ($_SESSION[leveluser]=='admin'){
      ...
      echo "<br><br><table id='table1' class='gtable sortable'><thead>
          <tr><th>No</th><th>Id kelas</th><th>Kelas</th><th>Aksi</th></tr></thead>";
    $no=1;
    while ($r=mysqli_fetch_array($tampil)){       
       echo "<tr><td>$no</td>
             ...
             echo "<td><a href='?module=kelas&act=editkelas&id=$r[id_kelas]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a> |
                 <a href=javascript:confirmdelete('$aksi?module=kelas&act=hapuskelas&id=$r[id_kelas]') title='Hapus'><img src='images/icons/cross.png' alt='Delete' /></a> |
    ...
    elseif ($_SESSION[leveluser]=='siswa'){
        ...
        $ambil_siswa = mysqli_query($con,"SELECT * FROM siswa WHERE id_siswa = '$_SESSION[idsiswa]'");
        $data_siswa = mysqli_fetch_array($ambil_siswa);
        $kelas = mysqli_query($con,"SELECT * FROM kelas WHERE id_kelas = '$data_siswa[id_kelas]'");
    ...
    elseif ($_SESSION[leveluser]=='pengajar'){
        echo "<form name='form_kelas' method=POST action='$aksi?module=kelas&act=input_walikelas'>
          ...
          <dt><label>Kelas </label></dt>    <dd> <select name='kelas' onChange='showsiswa()'>
                                      ...
                                      $tampilk = mysqli_query($con,"SELECT * FROM kelas");
                                      while($r=mysqli_fetch_array($tampilk)){
    ...
    case "editkelas":
    if ($_SESSION[leveluser]=='admin'){
    $tampil = mysqli_query($con,"SELECT * FROM kelas WHERE id_kelas = '$_GET[id]'");
    $r = mysqli_fetch_array($tampil);
    $getnip = mysqli_query($con,"SELECT * FROM pengajar WHERE id_pengajar = '$r[id_pengajar]'");
    $nipp = mysqli_fetch_array($getnip);
    $getnis = mysqli_query($con,"SELECT * FROM siswa WHERE id_siswa = '$r[id_siswa]'");
    ...
    echo "<form method=POST action='$aksi?module=kelas&act=update_kelas'>
          ...
          <dl class='inline'>
          <dt><label>Id Kelas</label></dt>       <dd> : <input type=text name='id_kelas' value='$r[id_kelas]' readonly> <br><small>Id tidak dapat diubah</small>   </dd>
          <dt><label>Nama Kelas</label></dt>     <dd> : <input type=text name='nama' value='$r[nama_kelas]'></dd>
          </dl>
          <div class='buttons'>
    ...
    elseif ($_SESSION[leveluser]=='pengajar'){
    $tampil = mysqli_query($con,"SELECT * FROM kelas WHERE id = '$_GET[id]'");
    $r = mysqli_fetch_array($tampil);
    ...
    <input type=hidden name=id value='$r[id]'>
          ...
          <dt><label>Kelas </label></dt>   <dd>: <select name='kelas' onChange='showsiswa()'>
                                      ...
                                      $tampilk = mysqli_query($con,"SELECT * FROM kelas WHERE id_pengajar ='0' ORDER BY id_kelas");
          ...
          <dt><label>Ketua Kelas</label></dt> <dd>:  <select name='ketua'>";
                                      $siswa = mysqli_query($con,"SELECT * FROM siswa WHERE id_siswa = '$r[id_siswa]'");
                                      ...
                                      $tampil_siswa = mysqli_query($con,"SELECT * FROM siswa WHERE id_kelas = '$r[id_kelas]'");
    ...
    elseif ($_SESSION[leveluser]=='siswa'){
         ...
         echo "<table>
          <tr><td>Kelas </td>   <td>: <select name='id_kelas'>
                                      <option value='$r[id_kelas]' selected>$r[nama]</option>";
                                      $tampilk = mysqli_query($con,"SELECT * FROM kelas ORDER BY id_kelas");
                                      while($t=mysqli_fetch_array($tampilk)){
                                            echo "<option value=$t[id_kelas]>$t[nama]</option>";
                                      }echo"</select></td></tr>
...
case "detailkelas":
    $detail=mysqli_query($con,"SELECT * FROM kelas WHERE id_kelas='$_GET[id]'");
   
    if ($_SESSION[leveluser]=='admin'){
    echo "<div class='information msg'>Detail Kelas</div>";
    echo "<br><table id='table1' class='gtable sortable'><thead>
          <tr><th>Id Kelas</th><th>Kelas</th><th>Aksi</th></tr></thead>";
    ...
    while ($r=mysqli_fetch_array($detail)){
       echo "<tr>
             <td>$no</td>
             <td>$r[nama]</td>";
             $getpengajar = mysqli_query($con,"SELECT * FROM pengajar WHERE id_pengajar = '$r[id_pengajar]'");
             ...
             $getsiswa = mysqli_query($con,"SELECT * FROM siswa WHERE id_siswa = '$r[id_siswa]'");
