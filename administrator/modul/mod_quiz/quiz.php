  ...
  default:
      ...
      if ($_SESSION[leveluser]=='admin'){    
        ...
        echo "<br><br><table id='table1' class='gtable sortable'><thead>
          <tr><th>no</th><th>Judul</th><th>Kelas</th><th>tgl Pembuatan</th><th>Pembuat</th><th>Waktu</th><th>Info</th><th>Terbit</th><th>aksi</th></tr></thead>";
        
        $tampil_topik = mysqli_query($con,"SELECT * FROM topik_quiz ORDER BY id_kelas");
        
    $no=1;
    while ($r=mysqli_fetch_array($tampil_topik)){
      $wpengerjaan = $r[waktu_pengerjaan] / 60;
      $tgl_buat   = tgl_indo($r[tgl_pembuatan]);
       echo "<tr><td>$no</td>
             <td>$r[judul]</td>";
             ...
             $k=mysqli_fetch_array($kelas);
                 echo "<td><a href=?module=kelas&act=detailkelas&id=$r[id_kelas] title='Detail Kelas'>$k[nama_kelas]</td>";
             }else{
                 echo"<td></td>";
             }
             echo"<td>$tgl_buat</td>";
             $pengajar = mysqli_query($con,"SELECT * FROM pengajar WHERE id_pengajar = '$r[id_pengajar]'");
             $cek_pengajar = mysqli_num_rows($pengajar);
             if(!empty($cek_pengajar)){
             $p=mysqli_fetch_array($pengajar);
                 echo "<td>$p[nama_pengajar]</td>";
             }else{
                 echo"<td></td>";
             }
             ...
             <td><ul><li><a href='?module=quiz&act=edittopikquiz&id=$r[id_tq]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a> | 
                 ...
                 <li><a href=?module=buatquiz&act=buatquiz&id=$r[id_topikquiz]>Buat Quiz</a></li>
                 <li><a href=?module=daftarquiz&act=daftarquiz&id=$r[id_topikquiz]>Daftar Quiz</a></li>
                 <li><a href=?module=quiz&act=daftarsiswayangtelahmengerjakan&id=$r[id_topikquiz]>Daftar Peserta & Koreksi</a></li></ul></td></tr>";
      $no++;
    }
    echo "</table>";
    ...
    $tampil_topik = mysqli_query($con,"SELECT * FROM topik_quiz WHERE pembuat = '$_SESSION[idpengajar]'");
        ...
        echo "<tr><td>$no</td>
             ...
             $pelajaran = mysqli_query($con,"SELECT * FROM mata_pelajaran WHERE id_matapelajaran = '$r[id_matapelajaran]'");
    ...
    elseif ($_SESSION[leveluser]=='siswa'){
        $siswa = mysqli_query($con,"SELECT * FROM siswa WHERE id_siswa = '$_SESSION[idsiswa]'");
        $data_siswa = mysqli_fetch_array($siswa);
        $mapel = mysqli_query($con,"SELECT * FROM mata_pelajaran WHERE id_kelas = '$data_siswa[id_kelas]'");
...
case "daftarsiswayangtelahmengerjakan":
    if ($_SESSION[leveluser]=='admin'){
        $siswa_yangmengerjakan = mysqli_query($con,"SELECT * FROM siswa_sudah_mengerjakan WHERE id_tq = '$_GET[id]'");
        ...
        $siswa_yangmengerjakan2 = mysqli_query($con,"SELECT * FROM siswa_sudah_mengerjakan WHERE id_tq = '$_GET[id]'");
        ...
        while ($t=mysqli_fetch_array($siswa_yangmengerjakan2)){
            $siswa = mysqli_query($con,"SELECT * FROM siswa WHERE id_siswa = '$t[id_siswa]'");
            $s = mysqli_fetch_array($siswa);
            $kelas = mysqli_query($con,"SELECT * FROM kelas WHERE id_kelas = '$s[id_kelas]'");
            $k = mysqli_fetch_array($kelas);
            $nilai = mysqli_query($con,"SELECT * FROM nilai_soal_esay WHERE id_tq='$_GET[id]' AND id_siswa ='$t[id_siswa]'");
            $n = mysqli_fetch_array($nilai);
            $nilai2 = mysqli_query($con,"SELECT * FROM nilai WHERE id_tq='$_GET[id]' AND id_siswa = '$t[id_siswa]'");
    ...
    elseif ($_SESSION[leveluser]=='pengajar'){
        $siswa_yangmengerjakan = mysqli_query($con,"SELECT id_siswa FROM siswa_sudah_mengerjakan WHERE id_tq = '$_GET[id]'");
        $cek_siswa = mysqli_num_rows($siswa_yangmengerjakan);

        if (!empty($cek_siswa)){

        $soal_pilganda = mysqli_query($con,"SELECT * FROM quiz_pilganda WHERE id_tq='$_GET[id]'");
        $pilganda = mysqli_num_rows($soal_pilganda);
        $soal_esay = mysqli_query($con,"SELECT * FROM quiz_esay WHERE id_tq='$_GET[id]'");
...
case "koreksi":
    if ($_SESSION[leveluser]=='admin'){
        $cek = mysqli_query($con,"SELECT * FROM siswa_sudah_mengerjakan WHERE id_siswa='$_GET[id_siswa]'");
        ...
        $esay = mysqli_num_rows($soal_esay);
        if (!empty($pilganda) AND !empty($esay)){
            $siswa = mysqli_query($con,"SELECT * FROM siswa WHERE id_siswa='$_GET[id_siswa]'");
            $s = mysqli_fetch_array($siswa);
            $jawaban = mysqli_query($con,"SELECT * FROM jawaban WHERE id_tq='$_GET[id]' AND id_siswa='$_GET[id_siswa]'");
            $cek=mysqli_num_rows($jawaban);
            if (!empty($cek)){
                echo "<h2>Jawaban Soal Essay Siswa <b>$s[nama_lengkap]</b></h2><hr>";
                $no=1;
                while ($j=mysqli_fetch_array($jawaban)){
                    $soal_esay2 = mysqli_query($con,"SELECT * FROM quiz_esay WHERE id_tq='$j[id_tq]' AND id_quiz='$j[id_quiz]'");
...
case "tambahtopikquiz":
    ...
    $pilih="SELECT * FROM kelas ORDER BY nama_kelas";
    $query=mysqli_query($con,$pilih);
    ...
    <option value=''>-pilih-</option>";
    $pilih= mysqli_query($con,"SELECT * FROM detail_pengajar WHERE id_pengajar ='$_SESSION[idadmin]'");
    while($row=mysqli_fetch_array($pilih)){
    $cari_kelas = mysqli_query($con,"SELECT * FROM kelas WHERE id_kelas = '$row[id_kelas]'");
...
case "edittopikquiz":
    ...
    $edit=mysqli_query($con,"SELECT * FROM topik_quiz WHERE id_tq = '$_GET[id]'");
    $t=mysqli_fetch_array($edit);
    $isikelas = mysqli_query($con,"SELECT * FROM kelas WHERE id_kelas = '$t[id_kelas]'");
    $k=mysqli_fetch_array($isikelas);
    $pelajaran = mysqli_query($con,"SELECT * FROM mata_pelajaran WHERE id_matapelajaran = '$t[id_matapelajaran]'");
    ...
    <dt><label>Kelas</label></dt>               <dd>: <select name='id_kelas' onChange='showpel_pengajar()'>
                                          <option value='".$k[id_kelas]."' selected>".$k[nama]."</option>";
                                          $pilih= mysqli_query($con,"SELECT DISTINCT id_kelas FROM mata_pelajaran WHERE id_pengajar ='$_SESSION[idpengajar]'");
                                          while($row=mysqli_fetch_array($pilih)){
                                          $cari_kelas = mysqli_query($con,"SELECT * FROM kelas WHERE id_kelas = '$row[id_kelas]'");
                                          while ($k=mysqli_fetch_array($cari_kelas)){
...
case "daftartopik":
    if ($_SESSION[leveluser]=='siswa'){
        $mapel = mysqli_query($con,"SELECT * FROM mata_pelajaran WHERE id_matapelajaran = '$_GET[id]'");
        $data_mapel = mysqli_fetch_array($mapel);
        $topik = mysqli_query($con,"SELECT * FROM topik_quiz WHERE id_kelas = '$_GET[id_kelas]' AND id_matapelajaran = '$_GET[id]' AND terbit='Y'");
        ...
        if (!empty($cek_topik)){
            echo"<br><b class='judul'>Daftar Tugas / Quiz Mata Pelajaran $data_mapel[nama]</b><br><p class='garisbawah'></p>
              ...
              while($t=mysqli_fetch_array($topik)){
                  ...
                  $pengajar =  mysqli_query($con,"SELECT * FROM pengajar WHERE id_pengajar = '$t[pembuat]'");
                  $cek_pengajar = mysqli_num_rows($pengajar);
                  $waktu = $t[waktu_pengerjaan] / 60;
...
case "nilaisiswa":
    if ($_SESSION[leveluser]=='siswa'){
        $quiz_pilganda = mysqli_query($con,"SELECT * FROM quiz_pilganda WHERE id_tq = '$_GET[id_topik]'");
        $quiz_esay = mysqli_query($con,"SELECT * FROM quiz_esay WHERE id_tq = '$_GET[id_topik]'");
        $c_pilganda = mysqli_num_rows($quiz_pilganda);
        $c_esay = mysqli_num_rows($quiz_esay);

        if (!empty($c_pilganda) AND !empty($c_esay)){
        $pilganda = mysqli_query($con,"SELECT * FROM nilai WHERE id_tq = '$_GET[id_topik]' AND id_siswa = '$_SESSION[idsiswa]'");
        $cek_pilganda = mysqli_num_rows($pilganda);
        $esay = mysqli_query($con,"SELECT * FROM nilai_soal_esay WHERE id_tq = '$_GET[id_topik]' AND id_siswa = '$_SESSION[idsiswa]'");
        ...
        $cek_esay = mysqli_num_rows($esay);
                ...
                elseif (empty($cek_esay)) {
                    $kerjakan = mysqli_query($con,"SELECT * FROM siswa_sudah_mengerjakan WHERE id_tq='$_GET[id_topik]' AND id_siswa = '$_SESSION[idsiswa]'");
...
case "infokerjakan":
    if ($_SESSION[leveluser]=='siswa'){
        $cek = mysqli_query($con,"SELECT * FROM siswa_sudah_mengerjakan WHERE id_tq='$_GET[id]' AND id_siswa = '$_SESSION[idsiswa]'");
        $data = mysqli_fetch_array($cek);
        
        if ($data[hits]<=0){
        $topik = mysqli_query($con,"SELECT * FROM topik_quiz WHERE id_tq = '$_GET[id]'");
...
case "buatquiz":
    if ($_SESSION[leveluser]=='admin'){
        $topik=mysqli_query($con,"SELECT * FROM topik_quiz WHERE id_topikquiz = '$_GET[id]'");
        $t=mysqli_fetch_array($topik);
        echo "<form><fieldset>
          ...
          <p align=center'>
          <input class='button white' type=button value='Buat Quiz Esay' onclick=\"window.location.href='?module=buatquizesay&act=buatquizesay&id=$t[id_topikquiz]';\">
          <input class='button white' type=button value='Buat Quiz Pilihan Ganda' onclick=\"window.location.href='?module=buatquizpilganda&act=buatquizpilganda&id=$t[id_topikquiz]';\">
          </p>
          <br><input class='button blue' type=button value=Kembali onclick=self.history.back()>
          </fieldset></form>";
    }
...
case "buatquizesay":
    if ($_SESSION[leveluser]=='admin'){
        $jum = mysqli_query($con,"SELECT COUNT(quiz_esay.id_quizesay) as jml FROM quiz_esay WHERE id_topikquiz = '$_GET[id]'");
...
case "buatquizpilganda":
    if ($_SESSION[leveluser]=='admin'){
        $jum = mysqli_query($con,"SELECT COUNT(quiz_pilganda.id_quiz) as jml FROM quiz_pilganda WHERE id_tq = '$_GET[id]'");
        $j = mysqli_fetch_array($jum);
        $jumlah = $j[jml] + 1;
...
case "daftarquiz":
    if ($_SESSION[leveluser]=='admin'){
        ...
        echo "<form><fieldset>
          ...
          <p align=center'>
          <input type=button class='button white' value='Daftar Quiz Esay' onclick=\"window.location.href='?module=daftarquizesay&act=daftarquizesay&id=$t[id_topikquiz]';\"> 
          <input type=button class='button white' value='Daftar Quiz Pilihan Ganda' onclick=\"window.location.href='?module=daftarquizpilganda&act=daftarquizpilganda&id=$t[id_topikquiz]';\">
          </p>
...
case "daftarquizesay":
    if ($_SESSION[leveluser]=='admin'){
        $cek = mysqli_query($con,"SELECT COUNT(quiz_esay.id_quizesay) as jml FROM quiz_esay WHERE id_topikquiz = '$_GET[id]'");
        $c = mysqli_fetch_array($cek);
        if ($c[jml] != 0){
        $quiz=mysqli_query($con,"SELECT * FROM quiz_esay WHERE id_topikquiz = '$_GET[id]'");        
        $jquiz = mysqli_query($con,"SELECT * FROM quiz_esay WHERE id_topikquiz = '$_GET[id]'");
        $jq = mysqli_fetch_array($jquiz);
        $topik = mysqli_query($con,"SELECT * FROM topik_quiz WHERE id_topikquiz = '$jq[id_topikquiz]'");
        $t=mysqli_fetch_array($topik);
        $kelas = mysqli_query($con,"SELECT * FROM kelas WHERE id_kelas = '$t[id_kelas]'");
        $k=mysqli_fetch_array($kelas);
        $pelajaran = mysqli_query($con,"SELECT * FROM mata_pelajaran WHERE id_matapelajaran = '$t[id_matapelajaran]'");
        $p = mysqli_fetch_array($pelajaran);
        ...
        echo "<div class='information msg'><b>Daftar Soal Quiz Esay</b></div>
              ...
              <dt><label>Kelas</label></dt>           <dd>: $k[nama_kelas]</dd>
              </dl></fieldset></form>";
        echo "<input class='button blue' type=button value='Tambah Quiz Esay' onclick=\"window.location.href='?module=quiz&act=buatquizesay&id=$jq[id_topikquiz]';\">
    ...
    while ($q=mysqli_fetch_array($quiz)){
       $tgl_buat   = tgl_indo($q[tgl_pembuatan]);
       echo "<tr><td>$no</td>
             <td>$q[pertanyaan]</td>
             <td>$q[gambar]</td>
             <td>$tgl_buat</td>
             <td><a href='?module=quiz&act=editquizesay&id=$q[id_quiz]&id_topik=$q[id_tq]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a> |
                 <a href=javascript:confirmdelete('$aksi?module=quiz&act=hapusquizesay&id=$q[id_quizesay]&id_topik=$q[id_topikq]uiz') title='Hapus' ><img src='images/icons/cross.png' alt='Delete' /></a>
                 </td></tr>";
      $no++;
    }
    echo "</table>";
    ...
    elseif ($_SESSION[leveluser]=='pengajar'){
        $cek = mysqli_query($con,"SELECT COUNT(quiz_esay.id_quiz) as jml FROM quiz_esay WHERE id_tq = '$_GET[id]'");
        $c = mysqli_fetch_array($cek);
        if ($c[jml] != 0){
        $quiz=mysqli_query($con,"SELECT * FROM quiz_esay WHERE id_tq = '$_GET[id]'");
        $jquiz = mysqli_query($con,"SELECT * FROM quiz_esay WHERE id_tq = '$_GET[id]'");
        $jq = mysqli_fetch_array($jquiz);
        $topik = mysqli_query($con,"SELECT * FROM topik_quiz WHERE id_tq = '$jq[id_tq]'");
        $t=mysqli_fetch_array($topik);
        $kelas = mysqli_query($con,"SELECT * FROM kelas WHERE id_kelas = '$t[id_kelas]'");
        $k=mysqli_fetch_array($kelas);
        $pelajaran = mysqli_query($con,"SELECT * FROM mata_pelajaran WHERE id_matapelajaran = '$t[id_matapelajaran]'");
...
case "daftarquizpilganda":
    if ($_SESSION[leveluser]=='admin'){
        $cek = mysqli_query($con,"SELECT COUNT(quiz_pilganda.id_quiz) as jml FROM quiz_pilganda WHERE id_tq = '$_GET[id]'");
        $c = mysqli_fetch_array($cek);
        if ($c[jml] != 0){
        $quiz=mysqli_query($con,"SELECT * FROM quiz_pilganda WHERE id_tq = '$_GET[id]'");
        $jquiz = mysqli_query($con,"SELECT * FROM quiz_pilganda WHERE id_tq = '$_GET[id]'");
        $jq = mysqli_fetch_array($jquiz);
        $topik = mysqli_query($con,"SELECT * FROM topik_quiz WHERE id_tq = '$jq[id_tq]'");
        $t=mysqli_fetch_array($topik);
        $kelas = mysqli_query($con,"SELECT * FROM kelas WHERE id_kelas = '$t[id_kelas]'");
...
case "editquizesay":
    if ($_SESSION[leveluser]=='admin'){
        $quiz=mysqli_query($con,"SELECT * FROM quiz_esay WHERE id_quiz = '$_GET[id]'");
...
case "editquizpilganda":
    if ($_SESSION[leveluser]=='admin'){
        $quiz=mysqli_query($con,"SELECT * FROM quiz_pilganda WHERE id_quiz = '$_GET[id]'");
        $q = mysqli_fetch_array($quiz);
