...
if ($module=='registrasi' AND $act=='hapus'){
   mysqli_query($con,"DELETE FROM registrasi_siswa WHERE id_registrasi='$_GET[id]'");
   header('location:../../media_admin.php?module='.$module);
}

elseif ($module=='registrasi' AND $act=='terima'){
    $registrasi = mysqli_query($con,"SELECT * FROM registrasi_siswa WHERE id_registrasi = '$_GET[id]'");
    $r=mysqli_fetch_array($registrasi);
    $pass = md5($r[nis]);
    mysqli_query($con,"INSERT INTO siswa(nis,nama_lengkap,username_login,password_login,id_kelas,alamat,tempat_lahir,
