...
<html>
	<head>
		<title>Pendaftaran E-Learning IONs International Education</title>
		<meta charset="utf-8">
		<link rel="stylesheet" type="text/css" href="administrator/css/style_register.css">
		<link rel="shortcut icon" type="image/x-icon" href="images/favicon.png7">
...
<script>
	...
	<body>
		...
		<div id="login2" class="box">
			...
			<section>
				<form method="POST" enctype="multipart/form-data" action="input_registrasi.php" onSubmit="return validasi(this)">
					<dl>
					...
					$kelas = mysqli_query($con,"SELECT * FROM kelas ORDER BY id_kelas");
					echo "<dd><select name='kelas'><option value='pilih' selected>--Pilih--</option>'";
					while ($k=mysqli_fetch_array($kelas)) {
						echo "<option value='$k[id_kelas]'>$k[nama]</option>";
					}
					echo "</select></dd>";
					?>
					<dt><label>Foto Profil :</label></dt>
					<dd><input type="file" name="gambar" size="40"><br>"*Tipe foto harus .jpg ukuran maks 300kb"<br>"**Foto boleh dikosongkan jika tidak ingin dipasang"</dd>
						<hr>
					</dl>
					<p>
